...
const HocOtSaleSort4 = (WrappedComponent) => {
    return (
        class HocOtSaleSort4 extends WrappedComponent {
            render() {
                return super.render();
            }
//弹窗部分
...
//编辑部分
            ...
            handleRowDeleteClickPic = (id, index) => {
                ...
                callAxios({
                    that: this,
                    method: 'delete',
                    url: `${API.saleSortBanner}?saleSortBannerId=${id.bannerId}&saleSortId=4`,
                })
                .then((response) => {
...
//数据交流
            fetchTableDataPic = (currentPage = 1, size = 10) => {
                ...
                callAxios({
                    that: this,
                    method: 'get',
                    url: `${API.saleSortBanner}?saleSortId=4&page=${currentPage - 1}&size=${size}${filter}`,
                })
                .then((response) => {
                    let {
                        data = [],
